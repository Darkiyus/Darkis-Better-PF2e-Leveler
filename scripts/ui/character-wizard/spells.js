...
export async function buildSpellContext(wizard) {
  ...
  if (primaryClassDef?.spellcasting) {
    spellSections.push(
      await buildCasterSpellSection(wizard, {
        target: 'primary',
  ...
  const { focusCantrips, focusNonCantrips } = await splitFocusSpells(renderedFocusSpells, {
    isDevotionChoice,
  });
  return {
    spellSubStep: 'focus',
...
async function buildCasterSpellSection(wizard, { target, classEntry, subclassEntry, classDef, classHandler }) {
  ...
  const focusContext = classHandler.buildFocusContext ? classHandler.buildFocusContext(sectionData, rawFocusSpells) : { focusSpells: rawFocusSpells, isDevotionChoice: classHandler.isFocusSpellChoice() };
  ...
  const grantedUuids = [...grantedSpells.cantrips.map((spell) => spell.uuid), ...grantedSpells.rank1s.map((spell) => spell.uuid)];
  ...
  const autoCurriculumUuids = [...((curriculum[0] ?? []).length <= 1 ? (curriculum[0] ?? []).slice(0, 1) : []), ...((curriculum[1] ?? []).length <= 2 ? (curriculum[1] ?? []).slice(0, 2) : [])];
  const curriculumSelectedUuids = [...selectedCurriculum.cantrips.map((spell) => spell.uuid), ...selectedCurriculum.rank1.map((spell) => spell.uuid)];
  const selectedUuids = new Set([...(sectionData.spells?.cantrips ?? []).map((spell) => spell.uuid), ...(sectionData.spells?.rank1 ?? []).map((spell) => spell.uuid), ...autoCurriculumUuids, ...curriculumSelectedUuids, ...grantedUuids, ...focusSpells.map((spell) => spell.uuid)]);
  ...
  const cantrips = allSpells.filter((spell) => spell.traits.includes('cantrip') && matchesTradition(spell) && matchesRarity(spell) && !selectedUuids.has(spell.uuid));

  const rank1Spells = allSpells.filter((spell) => !spell.traits.includes('cantrip') && spell.level === 1 && matchesTradition(spell) && matchesRarity(spell) && !selectedUuids.has(spell.uuid));
  ...
  const { focusCantrips, focusNonCantrips } = await splitFocusSpells(focusSpells, {
    isDevotionChoice,
  });
...
export function getSanitizedCurriculumSelections(wizard, target = 'primary') {
  ...
  return {
    cantrips: limitCurriculumSelections(sectionData.curriculumSpells?.cantrips ?? [], new Set(curriculum[0] ?? []), Math.min(1, (curriculum[0] ?? []).length)),
    rank1: limitCurriculumSelections(sectionData.curriculumSpells?.rank1 ?? [], new Set(curriculum[1] ?? []), Math.min(2, (curriculum[1] ?? []).length)),
  };
}
