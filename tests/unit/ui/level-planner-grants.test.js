...
describe('level planner grant previews', () => {
  ...
  test('widens selected Free Heart background skill choices even when PF2e text does not expose fallback prose', async () => {
    global.fromUuid = jest.fn(async (uuid) => {
      if (uuid === 'feat-free-heart') {
        return {
          uuid,
    ...
    expect(backgroundSkillChoice.sourceName).toBe('Martial Disciple');
    expect(backgroundSkillChoice.options).toEqual(expect.arrayContaining([
      expect.objectContaining({ value: 'arcana', disabled: false }),
    ]));
    expect(backgroundSkillChoice.options).not.toEqual(expect.arrayContaining([
  ...
  test('surfaces Fury Instinct bonus first-level barbarian feat choices from selected class feature options', async () => {
    const originalPacks = game.packs;
    const originalHas = game.i18n.has;
    const originalLocalize = game.i18n.localize;

    global.fromUuid = jest.fn(async (uuid) => {
      if (uuid === 'feature-instinct') {
        return {
          uuid,
          name: 'Instinct',
          type: 'classfeature',
          system: {
            rules: [{
              ...
              choices: [
                {
                  value: 'feature-fury-instinct',
                  label: 'Fury Instinct',
                  uuid: 'feature-fury-instinct',
                  slug: 'fury-instinct',
                },
              ],
            }],
    ...
    try {
      ...
      const context = await buildLevelContext(planner, ALCHEMIST, {});
      const instinct = context.classFeatures.find((entry) => entry.name === 'Instinct');
      const furyFeatChoice = instinct?.choiceSets.find((entry) => entry.flag === 'furyInstinct');
